...
/**
 * v15 -> v16: progressie op data, en het hardloopadvies eruit.
 *
 ...
 *
 * Deze stap kwam uit een andere tak dan v14 -> v15 en is daar bovenop gezet in plaats van
 * ermee samengevoegd. Versie 15 draaide al op een toestel, en een versienummer waarvan de
 * inhoud achteraf verandert is geen versienummer meer.
 *
 * Alles wat er verder staat — sessies, loops, streefgewichten, check-ins, instellingen —
 * blijft onaangeroerd, het advies uit v15 inbegrepen.
 */
function v15_to_v16(state: RawState): RawState {
  const users = (state.users ?? {}) as Record<string, unknown>
  const next: Record<string, unknown> = {}
